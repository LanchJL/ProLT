...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        target_probs = torch.zeros_like(log_probs).scatter_(-1, target.unsqueeze(-1), confidence)
        target_probs += self.smoothing / num_classes
...
def same_domain_triplet_loss(image_embedding, trip_images, gt, hard_k = None, margin = 2):
    ...
    batch_iterator = torch.arange(batch_size)
